<h1><u><?=$laguages[$default_lang]['tyre_delivery'];?></u></h1>
...
<?php
  ...
  $query_protocols = "SELECT `tyres_storages`.*, `warehouses`.`warehouse_name`,`vehicles_types`.`vehicle_type`,`vehicles_models`.`vehicle_model`,
                      ...
                      WHERE `tyres_storages`.`tyre_storage_date` >= '$expired_date'
                      ORDER BY `tyres_storages`.`tyre_storage_date` DESC, `tyres_storages`.`tyre_storage_id` DESC";
...
<div class="search">
  ...
  <table>
    <tbody>
      <tr>
        <td width="10%"><input type="text" id="search_tyre_storage_id"></td>
...
?>
        ...
        <td width="10%"><input type="text" id="search_tyre_storage_date" class="datepicker"></td>
        <td width="5%"><button class="button" onClick="SearchProtocolForDelivery()"><?=$laguages[$default_lang]['btn_search'];?></button></td>
        <td width="9%"></td>
      </tr>
    </tbody>
  </table>
